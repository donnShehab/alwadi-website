  ...
  return (
    <section id="products" className="bg-brand-white section-padding overflow-hidden">
      <div className="max-width-container">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 sm:mb-32 gap-8 sm:gap-10">
           ...
           <p className="text-lg sm:text-xl text-brand-gray-dark/50 font-medium max-w-sm leading-tight pb-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-20 gap-y-20 sm:gap-y-40">
           {categories.map((cat, i) => (
             ...
             >
                ...
                <div className="relative aspect-[4/5] overflow-hidden rounded-[2.5rem] sm:rounded-[3rem] shadow-2xl group border-[6px] sm:border-[8px] border-white bg-brand-gray-light">
                   <Image 
                    src={cat.image} 
                    alt={cat.title} 
                    fill 
                   ...
                   {/* Overlay Number */}
                   <span className="absolute top-8 right-8 sm:top-10 sm:right-10 text-6xl sm:text-8xl font-black text-white/20 select-none group-hover:text-white/40 transition-colors">
                     {cat.number}
                   </span>
                </div>

                {/* Content Details */}
                <div className="mt-8 sm:mt-12 space-y-4 sm:space-y-6 px-2 sm:px-4 interactive">
                   <div className="flex items-center gap-4">
                      <span className="text-[10px] font-black uppercase tracking-[0.3em] text-brand-orange">{cat.highlight}</span>
                      <div className="h-px w-8 sm:w-10 bg-brand-gray-mid" />
                   </div>
                   <h3 className="text-2xl sm:text-4xl">{cat.title}</h3>
                   <p className="text-base sm:text-lg text-brand-gray-dark/60 font-medium leading-relaxed max-w-sm">
                     {cat.description}
                   </p>
